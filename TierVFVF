...
TIEMPOINSTALACIONCHURNERS AS(
  SELECT DISTINCT i.NOMBRE_CONTRATO as CONTRATO, EXTRACT (MONTH FROM i.FechaInst) AS MES, DATE_DIFF (i.FechaInst, i.FechaApertura, DAY) AS TIEMPOINSTCHURN
    FROM INSTALACIONES2021 i INNER JOIN ALTAS a ON i.NOMBRE_CONTRATO = a.CONTRATO INNER JOIN CRUCECHURNERS c on i.NOMBRE_CONTRATO = c.CONTRATOCRM
    --WHERE
    --c.Maxfecha > i.FechaInst AND DATE_DIFF(c.maxfecha, i.FechaInst, DAY) <= 60
    --AND 
    --a.fecha>=i.FechaInst AND date_diff(i.fechainst, a.fecha, day)<=30
